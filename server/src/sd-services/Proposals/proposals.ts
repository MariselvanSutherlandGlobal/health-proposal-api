...
export class proposals {
  ...
  private mountAllPaths() {
    ...
    this.app['post'](
      `${this.serviceBasePath}/caseCreateTest`,
      cookieParser(),
      this.sdService.getMiddlesWaresBySequenceId(
        null,
        'pre',
      ...
      async (req, res, next) => {
        ...
        try {
          ...
          //appendnew_next_sd_C284MxVsBp28fNXn
        } catch (e) {
          return await this.errorHandler(bh, e, 'sd_C284MxVsBp28fNXn');
        }
      },
      this.sdService.getMiddlesWaresBySequenceId(
        null,
    ...
    this.app['post'](
      `${this.serviceBasePath}/caseCreateTests`,
      cookieParser(),
      this.sdService.getMiddlesWaresBySequenceId(
        null,
        'pre',
      ...
      async (req, res, next) => {
        ...
        try {
          ...
          let parentSpanInst = null;
          bh = await this.prepareCasePayloadDynamic(bh, parentSpanInst);
          //appendnew_next_sd_BynggDPTF37hrm6U
        } catch (e) {
          return await this.errorHandler(bh, e, 'sd_BynggDPTF37hrm6U');
        }
      },
      this.sdService.getMiddlesWaresBySequenceId(
        null,
...
--------------------------------------*/
    ...
    try {
      bh.local.casePayload = {
        ...
        caseData: {
          ...
          ProposerDOB: '18/11/2000',
          SpouseDOB: '11/11/2002',
          SpouseIncluded: true,

          TobaccoUse: true,
          DiabetesDeclared: true,
          HypertensionDeclared: true,
      ...
      const accessToken = 'iTt-BWsNOvudxh0Wl4xNGlRA2bpICF-M8DBl3J9AmBV';
      ...
      console.log(
        'Headers:',
        JSON.stringify(bh.local.caseRequest.headers, null, 2)
      );
  ...
  async caseCreate(bh, parentSpanInst) {
    try {
      ...
      bh = await this.sd_mRDl8ismOqS863X6(bh, parentSpanInst);
      //appendnew_next_caseCreate
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_pqhKSHDFYo2mVbzn');
    }
  }

  async sd_mRDl8ismOqS863X6(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_mRDl8ismOqS863X6',
      parentSpanInst
    );
    try {
      console.log('========== ALPHA CASE RESPONSE ==========');

      console.log(JSON.stringify(bh.local.caseResponse, null, 2));
      this.tracerService.sendData(spanInst, bh);
      await this.proposalHttpOut(bh, parentSpanInst);
      //appendnew_next_sd_mRDl8ismOqS863X6
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async prepareCasePayloadDynamic(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'prepareCasePayloadDynamic',
      parentSpanInst
    );
    try {
      const body = bh.input.body;

      console.log('========== CASE CREATE TEST REQUEST ==========');
...
==================================================*/
      ...
      if (!body.case_payload) {
        throw new Error('case_payload is missing');
      }
...
==================================================*/
      ...
      bh.local.casePayload = body.case_payload;
...
==================================================*/
      ...
      console.log('========== ALPHA CASE REQUEST ==========');
      ...
      console.log('Body:', JSON.stringify(bh.local.caseRequest.body, null, 2));
      this.tracerService.sendData(spanInst, bh);
      bh = await this.caseCreates(bh, parentSpanInst);
      //appendnew_next_prepareCasePayloadDynamic
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async caseCreates(bh, parentSpanInst) {
    try {
      let requestOptions: any = {
        url: bh.local.caseRequest.url,
        timeout: 30000,
      ...
      bh.local.caseResponse = responseMsg;
      bh = await this.sd_XNoy68As6NOjYTPw(bh, parentSpanInst);
      //appendnew_next_caseCreates
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_0uZHzmIIyfpnBelE');
    }
  }

  async sd_XNoy68As6NOjYTPw(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_XNoy68As6NOjYTPw',
      parentSpanInst
    );
    try {
      ...
      console.log(JSON.stringify(bh.local.caseResponse, null, 2));
      this.tracerService.sendData(spanInst, bh);
      await this.proposalHttpOuts(bh, parentSpanInst);
      //appendnew_next_sd_XNoy68As6NOjYTPw
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async proposalHttpOuts(bh, parentSpanInst) {
    try {
      bh.web.res.status(200).send(bh.local.response);
